...
clean='function cleanTime(v){return v.replace(/\\s+/g,"").replace("(+ 1)","(+1)")}\n'
helpers=r'''function cleanTime(v){return v.replace(/\s+/g,"").replace("(+ 1)","(+1)")}
...
'''
replace_once(clean,helpers,'insert operational-date helpers')
...
''',
'preserve activity source date/time')
...
'''    rows.push({
      ...
      _sourceDutyStart:dutyStart,
''',
'preserve flight source date/time')
...
''',
'keep PDF evidence on source date')
...
'''  allRows=restoreMissingPilotDates(
  ...
  allRows=applyPilotHotelAssignments(allRows,combinedText);
''',
'normalize parsed rows before calendar fill')
...
''',
'calendar grouping uses operational date')
...
'''    const date=String(operationalRosterDate(row)||"").trim();
    if(!date) return;
''',
'statistics group by operational date')
...
''',
'keep scheduled UTC arithmetic on source date')
...
''',
'keep Smart Duty UTC arithmetic on source date')

app_path.write_text(app)

index=Path('index.html')
index.write_text(index.read_text().replace('?v=184','?v=185'))

sw=Path('sw.js')
sw.write_text(sw.read_text().replace('crewview-v184','crewview-v185').replace('?v=184','?v=185'))

regression=r'''const fs=require('node:fs');
...
'''
Path('tests/operational-date-boundary.cjs').write_text(regression)
